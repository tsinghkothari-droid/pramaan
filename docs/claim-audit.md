...
| CLAIM-STATUS-CAP-019 | `STATUS.md` | STATUS: Real Hypothesis/fast-check adapters | executable-test | `crates/pramaan-cli/src/fuzz.rs`, `scripts/check-fuzz-harness-evidence.mjs` | `cargo test -p pramaan-cli fuzz` | pass-with-risk | Tool-backed mode depends on installed Hypothesis/fast-check; missing tools must remain visible. |
...
| CLAIM-STATUS-CAP-020A | `STATUS.md` | STATUS: AI evidence-seeking probe plan and bounded execution | executable-test | `crates/pramaan-cli/src/main.rs`, `crates/pramaan-cli/tests/smoke.rs`, `schemas/probe.schema.json`, `docs/ai-probe-generator.md`, `scripts/check-ai-probe-execution.mjs` | `cargo test --workspace && node scripts/check-ai-probe-execution.mjs <executed-plan.json>` | pass-with-risk | Bounded safe-marker execution exists; arbitrary generated-code execution and mutation/differential validation of accepted probes remain future work. |
