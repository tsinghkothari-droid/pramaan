# Research Index

This index points to the research files that define Pramaan's current direction.

## Core Research

- [.planning/research/SUMMARY.md](../.planning/research/SUMMARY.md)
- [.planning/research/IMPROVEMENT_RESEARCH_2026-05-18.md](../.planning/research/IMPROVEMENT_RESEARCH_2026-05-18.md)
- [.planning/research/TOP_100_FLAWS_AND_MITIGATIONS_2026-05-18.md](../.planning/research/TOP_100_FLAWS_AND_MITIGATIONS_2026-05-18.md)

## Product Architecture Research

- [.planning/research/STACK.md](../.planning/research/STACK.md)
- [.planning/research/FEATURES.md](../.planning/research/FEATURES.md)
- [.planning/research/ARCHITECTURE.md](../.planning/research/ARCHITECTURE.md)
- [.planning/research/PITFALLS.md](../.planning/research/PITFALLS.md)

## Important External Anchors

- SLSA: https://slsa.dev/spec/
- Sigstore: https://docs.sigstore.dev/cosign/signing/overview/
- GitHub artifact attestations: https://docs.github.com/en/actions/concepts/security/artifact-attestations
- Hypothesis: https://hypothesis.readthedocs.io/en/latest/reference/api.html
- fast-check: https://fast-check.dev/docs/introduction/why-property-based/
- StrykerJS: https://stryker-mutator.io/docs/stryker-js/incremental/
- mutmut: https://mutmut.readthedocs.io/en/latest/
- cargo-mutants: https://mutants.rs/timeouts.html
- CodeHalu: https://arxiv.org/abs/2405.00253
- Delulu: https://arxiv.org/abs/2605.07024
- SWE-bench Verified analysis: https://openai.com/index/why-we-no-longer-evaluate-swe-bench-verified/

## Research Rule

When new research changes the product shape, update:

1. `.planning/research/`
2. `.planning/REQUIREMENTS.md`
3. `.planning/ROADMAP.md`
4. relevant `.planning/phases/*`
5. this index
